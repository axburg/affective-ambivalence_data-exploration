...
Merkel_M1 <- lm(rescale(mean_amb_Merkel) ~ rescale(mean_symb_id), data = wkp17)
Merkel_M2 <- lm(rescale(mean_amb_Merkel) ~ rescale(mean_symb_id) + gend + age + edu + region + rescale(mean_pol_int), data = wkp17)

##Schulz

Schulz_M1 <- lm(rescale(mean_amb_Schulz) ~ rescale(mean_symb_id), data = wkp17)
Schulz_M2 <- lm(rescale(mean_amb_Schulz) ~ rescale(mean_symb_id) + gend + age + edu + region + rescale(mean_pol_int), data = wkp17)

##Table OA1

tab_model(Merkel_M1, Merkel_M2,
          Schulz_M1, Schulz_M2,
...
plot_grid(Fig2a, Fig2b, Fig2c, Fig2d, labels = c('A', 'B', 'C', 'D'), label_size = 16)L
...
cor.test(wkp17$mean_econ_id, wkp17$mean_amb_Merkel, use = "complete.obs")

cor.test(wkp17$mean_econ_id, wkp17$mean_amb_Schulz, use = "complete.obs")

cor.test(wkp17$mean_soc_id, wkp17$mean_amb_Merkel, use = "complete.obs")

cor.test(wkp17$mean_soc_id, wkp17$mean_amb_Schulz, use = "complete.obs")
...
FigA1b <- ggplot(wkp17, aes(x = mean_soc_id, y = mean_att_Merkel)) + 
  ...
  annotate("segment", x = 2.5, xend = 4.5, y = 4.0, yend = 5.0, colour = "red", size = 1.0, alpha = 0.6, arrow = arrow()) +
  annotate("segment", x = 5.4, xend = 7.2, y = 5.0, yend = 4.5, colour = "red", size = 1.0, alpha = 0.6, arrow = arrow()) +
  annotate("text", x = c(3.5, 6.8), y = c(3.2, 3.2), label = c("b = 0.18***", "b = - 1.57***"), color = "red", size = 4, fontface = "bold")
...
wkp17$Merkel_amb_dif <- wkp17$Merkel_amb_w6 - wkp17$Merkel_amb_w4
wkp17$Schulz_amb_dif <- wkp17$Schulz_amb_w6 - wkp17$Schulz_amb_w4

t.test(wkp17$Merkel_amb_dif, mu = 0) # < 0
t.test(wkp17$Schulz_amb_dif, mu = 0) # < 0

cor.test(wkp17$mean_symb_id, wkp17$Merkel_amb_dif, use = "complete.obs") # r = -.02
cor.test(wkp17$mean_symb_id, wkp17$Schulz_amb_dif, use = "complete.obs") # ns

a = twolines(Merkel_amb_dif ~ mean_symb_id, data = wkp17) # ns
a = twolines(Schulz_amb_dif ~ mean_symb_id, data = wkp17) # ns
