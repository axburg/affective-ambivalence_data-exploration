...
Merkel_M1a <- lm(rescale(mean_amb_Merkel) ~ rescale(mean_symb_id) + I(rescale(mean_symb_id)^2), data = wkp17)
Merkel_M1b <- lm(rescale(mean_amb_Merkel) ~ rescale(mean_symb_id) + I(rescale(mean_symb_id)^2) + rescale(mean_att_Merkel) + rescale(mean_pol_int) + gend + age + edu + region, data = wkp17)

##Schulz

Schulz_M1a <- lm(rescale(mean_amb_Schulz) ~ rescale(mean_symb_id) + I(rescale(mean_symb_id)^2), data = wkp17)
Schulz_M1b <- lm(rescale(mean_amb_Schulz) ~ rescale(mean_symb_id) + I(rescale(mean_symb_id)^2) + rescale(mean_att_Schulz) + rescale(mean_pol_int) + gend + age + edu + region, data = wkp17)

tab_model(Merkel_M1a, Merkel_M1b,
          Schulz_M1a, Schulz_M1b,
          show.intercept = FALSE,
          show.ci = FALSE,
          order.terms = c(1, 2, 3, 10, 4, 5, 6, 7, 8, 9),
...
a = twolines(mean_amb_Merkel ~ mean_symb_id + mean_att_Merkel + mean_pol_int + gend_num + age + edu_d1 + edu_d2 + as.numeric(region), data = wkp17)
...
Fig1b <- ggplot(wkp17, aes(x = mean_symb_id, y = mean_amb_Schulz)) + 
  geom_jitter(alpha = 0.02) + 
  geom_smooth() +
  labs(x = "Left-Right Self-placement",
       y = "Ambivalence") +
...
a = twolines(mean_amb_Schulz ~ mean_symb_id + mean_att_Schulz + mean_pol_int + gend_num + age + edu_d1 + edu_d2 + as.numeric(region), data = wkp17)

cowplot::plot_grid(Fig1a, Fig1b, nrow = 2, labels = c('Merkel', 'Schulz'), label_size = 16)
