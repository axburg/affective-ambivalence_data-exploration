...
Scholz_M1a <- lm(rescale(Scholz_aamb) ~ rescale(symb_id), data = rcs21)
Scholz_M1b <- lm(rescale(Scholz_aamb) ~ rescale(symb_id) + gend + age + edu + region + rescale(pol_int), data = rcs21)

#Cognitive Ambivalence

Scholz_M2a <- lm(rescale(Scholz_camb) ~ rescale(symb_id), data = rcs21)
Scholz_M2b <- lm(rescale(Scholz_camb) ~ rescale(symb_id) + gend + age + edu + region + rescale(pol_int), data = rcs21)
